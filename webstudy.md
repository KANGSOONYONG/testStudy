...
* 모바일은 화면이 작다.
* 기기의 성능이 상대적으로 좋지 않다
출처 : https://velog.io/@lllen/%EB%AA%A8%EB%B0%94%EC%9D%BC-%EC%9B%B9%EA%B0%9C%EB%B0%9C

### 자바 웹 개발 환경에서부터 지금의 프론트엔드 개발 환경까지

* html 은 정적인 파일(내용을 직접 수정하기전까지 변하지 않음)이므로 시시각각 변하는 뉴스기사 라든가 날씨정보, 쇼핑몰의 상품 정보등을 가지고 있을 수 없다. 이러한 정보들은 데이터베이스에 저장되어 있으며 웹을 통해 정보를 제공하려면 사용자의 웹 요청을 받아 별도의 프로그램이 동작해야 하고 데이터베이스에서 가지고온 정보들은 html 로 재구성해서 클라이언트에 다시 전달해야 한다. <br> 
이외에도 사용자 로그인 처리를 위해서는 입력한 아이디와 비밀번호등을 데이터베이스에 저장된 내용과 비교해야 하고 게시판의 경우 입력한 내용을 데이터베이스에 저장해야 하는 과정이 필요 하게 된다.
* 서블릿은 바로 이러한 기능을 수행할 수 있도록 설계된 특수한 목적의 ```자바 프로그램```으로 이해할 수 있다.
* 하지만 서블릿은 코드 형태가 비효율적이기 때문에, 문제 해결을 위해 JSP(Java Server Pages)가 등장하였다.
* JSP 또한 단점이 존재하긴 한다.. 몇몇 템플릿 엔진 이라고 하는 JSP 대체 솔루션들이 나와 여러 문제를 개선하기는 했지만 2010년 이후 스마트폰과 같은 모바일 개발이 주류가 되고 자바스크립트 기술의 발전으로 프론트엔드 개발이 새로운 트랜드가 된 이후에는 전통적인 JSP 기반의 개발은 다소 성장세를 잃어가고 있다고 볼 수 있다.

* 아래와 같이, 새로운 요구에 따라 REST API 방식의 데이터 교환이 보편화 되고 자바스크립트 기술의 발전은 서버로 부터 데이터만 수신하고 클라이언트(웹브라우저)에서 데이터를 바인딩한 다음 화면을 렌더링(CSR, Client Side Rendering) 하는 방식으로 개발을 전환하게 되는 계기가 되었다. 이러한 개발 방식을 프론트엔드 중심 개발이라고 하고 대표적인 자바스크립트 기반 기술로 angular, react.js, vue.js 등이 주류를 이루고 있다.
...
``` 
이 부분의 글은 요약된 글입니다. 좀 더 자세한 내용을 알고 싶다면 아래 출처로 이동하세요 !
 출처 : https://dinfree.com/lecture/backend/javaweb_1.1.html
```

### 모바일 UI·UX 디자인시 고려해야 할 가이드라인 1 (출처 : https://brunch.co.kr/@chulhochoiucj0/8)
```모바일과 웹에서의 디자인은 많은 부분이 다릅니다. 디자인 전, 각 device의 특성과 사용 맥락의 차이를 알아야 합니다.```
* 우선 모바일과 데스크톱은 화면 크기에서부터 차이가 난다. 저의 경우 데스크톱은 실내에서 사용하고 집중이 필요한 업무를 할 때 주로 사용합니다. 모바일은 대중교통 이용할 때 또는 간단하게 잠깐 무언가를 확인할 때 많이 쓰는 것 같아요
* 이렇게 사용 환경에 따라 집중도가 달라지고, 모바일 기기에서 텍스트를 이해하는 것이 데스크톱에 비해 두 배 가량 힘들어진다고 합니다.
* 또한 모바일 사용 환경은 데스크톱와 달리 손가락으로 거의 모든 것을 해결합니다. 이처럼 사용 맥락상 많은 차이점이 있음에도 단순히 웹 화면크기를 줄여서 디자인된 모바일 웹과 앱이 아직도 많이 존재한다고 합니다.  
* 아래 링크를 타고 넘어가서 모바일에서의 최적의 UX 디자인을 하기 위해 고려해야 할 사항이 무엇이 있는지 참고하면 모바일 앱 디자인에 도움이 많이 될 것 같아요. <br>


확인하러가기 : https://brunch.co.kr/@chulhochoiucj0/8
