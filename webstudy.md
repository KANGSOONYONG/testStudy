...
* 우선 모바일과 데스크톱은 화면 크기에서부터 차이가 난다. 저의 경우 데스크톱은 실내에서 사용하고 집중이 필요한 업무를 할 때 주로 사용합니다. 모바일은 대중교통 이용할 때 또는 간단하게 잠깐 무언가를 확인할 때 많이 쓰는 것 같아요. <br>
이렇게 사용 환경에 따라 집중도가 달라지고, 모바일 기기에서 텍스트를 이해하는 것이 데스크톱에 비해 두 배 가량 힘들어진다고 합니다.  <br>
또한 모바일 사용 환경은 데스크톱와 달리 손가락으로 거의 모든 것을 해결합니다. 이처럼 사용 맥락상 많은 차이점이 있음에도 단순히 웹 화면크기를 줄여서 디자인된 모바일 웹과 앱이 아직도 많이 존재한다고 합니다.  <br>

따라서 아래 링크를 타고 넘어가서 모바일에서의 최적의 UX 디자인을 하기 위해 고려해야 할 사항이 무엇이 있는지 참고하면 모바일 앱 디자인에 도움이 많이 될 것 같아요.
